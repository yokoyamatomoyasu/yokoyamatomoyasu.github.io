...
2023年の中で最も読んでおくべき論文と思うのはやはりDeepMindさんが計算で38万もの合成可能性の高い材料を予測した論文です。

これまで計算で熱力学的に安定とされた材料は高々数万件でしたが、この論文により1桁も材料データ空間を広げたのはすごいです。
私はボストンで開催された2023　MRS fall meetingにてこの発表を立ち見で聞いていたのですが、出てきた数字に「？？？」とりました。
...
もちろんやるべきことはまだまだあり、例えば報告のない未知構造はあまり考慮されていない点、第一原理計算での予測が困難なランタノイドを含む化合物が多い点などの課題はあると個人的には思います。
...
GPT−4から予測された実験条件を人間が実際に試し、その結果をGPT-4にフィードバックすることを通じて、これまで報告のない金属有機構造体（MOF）を合成できたという内容で、LLMが予測した材料をちゃんと合成して確認したのはこれが初めてだと思います。
...
これまでは遺伝的アルゴリズムを使って最安定な構造へ最適化する手法がよく使われてきましたが、2018年ごろに敵対的生成ネットワーク（GAN）や変分オートエンコーダ（VAE）などの生成AIを使った手法が提案されて以降この分野の研究者人口が増加し、今年のMRSでも多くの発表がありました。
...
マルチモーダルAIとは異なる種類の情報をまとめて扱うAIのことを指します。 
例えば、分子を表現する際には、テキスト形式やグラフ形式、画像形式、あるいはスペクトル形式など、材料科学でもいくつかの種類の情報があります。
...
この論文は発想がすごいですし手法も作り込まれていて、個人的に2023年で一番面白かった論文でした。

MD法は原子の物理的な動きの時間経過を計算する手法であり、材料だけでなく創薬のシミュレーションにおいても最も使われる手法の一つです。
近年は機械学習ポテンシャルを用いることでどんな材料でも高速かつ高精度に分子動力学ができるようになり、今年も毎日のようにそうした論文を目にしました。
しかし、機械学習ポテンシャルを用いても平衡状態まで到達するのには時間がかかります。
...
上記の５本を選ぶ際に悩んだ論文その1は、量子アニーリングを結晶構造予測に応用した論文です。
...
<blockquote class="twitter-tweet"><p lang="ja" dir="ltr">真面目に読んでみました。<br><br>結晶構造予測を整数計画問題に落とし込み量子アニーリングで最適化したという内容。<br><br>気になった点は、<br><br>・空間群は入力する必要がある<br>・仮定したポテンシャルが単純<br><br>でまだまだ実用的ではないけど、量子技術が進展したら楽しみだなと感じました。 <a href="https://t.co/GjuJUmNeyN">https://t.co/GjuJUmNeyN</a></p>&mdash; 横山トモヤス｜計算材料科学者 (@yoko_materialDX) <a href="https://twitter.com/yoko_materialDX/status/1677995208921989120?ref_src=twsrc%5Etfw">July 9, 2023</a></blockquote> <script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>
...
上記の５本を選ぶ際に悩んだ論文その2は、深層シンボリック回帰により物理法則を発見する論文です。
...
番外編その4は物理情報付きニューラルネットワーク（PINN）のレビュー論文。
