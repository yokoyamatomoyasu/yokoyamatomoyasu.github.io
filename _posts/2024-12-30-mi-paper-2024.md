...
※ここでの「機械学習ハミルトニアン」は、DM21などのいわゆる「機械学習汎関数」と区別するために別の言葉を用いています。機械学習汎関数は第一原理計算における電子密度とエネルギーの正確な関係性がわからない汎関数（交換相関汎関数）をニューラルネットワークにより表現するのに対し、ここで紹介する研究はそれを含むいくつかの汎関数により表される電子のハミルトニアン自体を直接予測するため、著者らの表現に倣い「機械学習ハミルトニアン」と表現しました。
