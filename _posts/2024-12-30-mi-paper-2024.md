...
※本文における「機械学習ハミルトニアン」は、いわゆる「機械学習汎関数」とは違う意味として定義しています。機械学習汎関数は第一原理計算における正確な形がわからない汎関数（交換相関汎関数）をニューラルネットワークとして表現するのに対し、今回紹介する論文はそれを含むいくつかの汎関数により表される電子のハミルトニアン自体を直接予測するため、著者らの表現方法に倣い「機械学習ハミルトニアン」という言葉を用いています。
